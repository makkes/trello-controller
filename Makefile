...
IMG ?= $(IMG_REGISTRY)/makkes/trello-controller:$(IMG_TAG)
...
DOCKER_BUILD_LABELS ?= --label org.opencontainers.image.created=$(shell date -u +"%Y-%m-%dT%H:%M:%SZ") \
	--label org.opencontainers.image.authors="Max Jonas Werner <max@e13.dev>" \
	--label org.opencontainers.image.source=https://github.com/makkes/trello-controller \
	--label org.opencontainers.image.revision=$(IMG_TAG) \
	--label org.opencontainers.image.vendor="Max Jonas Werner"
...
.PHONY: artifact-push
artifact-push:
	flux push artifact oci://ghcr.io/makkes/manifests/trello-controller:$(IMG_TAG) --path=./config/ --source=https://github.com/makkes/trello-controller --revision=$(IMG_TAG)/$(shell git rev-parse HEAD)
